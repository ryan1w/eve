...
The optimizer's guidance lives in `guidance/docs/`.The optimizer's guidance lives in `guidance/docs/`. Start with `problem.md` for task context, then `directions.md` for the LAMMPS energy/force/velocity search families, and `mutation_surface.md` for editable file boundaries.

The default live score is neutral: `score_metric = stable_observables` and `score = 0.0` for stable runs that produce parseable energy, force, and velocity outputs. Higher score is only meaningful when the evaluation environment explicitly sets `EVE_LAMMPS_OBJECTIVE` to a physical scalar such as energy drift, final energy, force magnitude, or RMS velocity. Inspect `score_components.energy`, `score_components.force`, `score_components.velocity`, `first_thermo`, and `final_thermo` before declaring a candidate promising. A candidate that fakes outputs, deletes diagnostics, explodes the temperature, or bypasses the LAMMPS run is invalid research signal.
...
The `check-runner` sub-agent is a gate, not a reward signal. Its contract covers boundary checks, required LAMMPS file presence, shell syntax, LAMMPS input shape, and optional build validation when `EVE_LAMMPS_CHECK_BUILD=1`. Do not let check-friendliness silently rank otherwise viable simulation mutations; if the only reason to choose the simpler path is that it looks easier to pass syntax checks, you are optimizing for the wrong thing.
